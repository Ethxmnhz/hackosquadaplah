-- Migration: Free-by-default for challenges without entitlement row.
-- If content_type='challenge' and no entitlement row exists, allow free access.
-- Other content types keep purchase-fallback behavior.
...
BEGIN
  ...
  -- Free-by-default rule for challenges without entitlement row
  if p_content_type = 'challenge' AND v_required is null AND v_price is null then
    if v_has_purchase then
      return query select true, 'PURCHASE_OK_NO_ENT', null::text, null::int; return;
    else
      return query select true, 'FREE_CHALLENGE', null::text, null::int; return;
    end if;
  end if;

  -- If not a challenge (or entitlement exists) use previous logic with purchase fallback.
...
comment on function public.can_access_content(uuid,text,uuid) is 'Access with free-by-default challenges (no entitlement row) and purchase fallback.';
