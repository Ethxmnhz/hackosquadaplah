-- Migration: Force drop & recreate admin_grant_plan (handles case where prior DO block failed to drop due to signature mismatch)
-- Timestamp: 2025-09-28 17:10:00

do $$
begin
  begin
    execute 'drop function if exists public.admin_grant_plan(text,text,text)';
  exception when others then
    -- ignore drop errors; proceed to recreate
    null;
  end;
end$$;
