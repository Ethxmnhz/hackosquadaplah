...
const OperationPage = () => {
  ...
  <div className="relative min-h-screen text-white overflow-hidden" style={{ background: 'radial-gradient(ellipse at 60% 20%, #181024 0%, #0A030F 100%)' }}>
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <video
          ...
          loop
          muted
          className="w-full h-[38vh] min-h-[420px] object-cover brightness-[.18]"
  ...
  <div className="text-center mb-16 relative z-10">
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-8 bg-gradient-to-r from-red-400 via-primary to-blue-400 text-transparent bg-clip-text drop-shadow-[0_2px_24px_rgba(127,90,240,0.25)] animate-fade-in">
            <span className="block border-b-4 border-primary/60 pb-2 mb-2">Practice the Past.</span>
            <span className="block">Defend the Future.</span>
          </h1>
          <p className="text-2xl md:text-3xl text-white max-w-3xl mx-auto mb-10 animate-fade-in font-semibold" style={{textShadow:'0 2px 16px #7f5af0cc'}}>
            <span className="text-primary">Relive</span> the world’s most <span className="text-accent-blue">infamous breaches</span>. <span className="text-primary">Play</span> with friends. <span className="text-red-400">Make enemies</span>. <span className="text-accent-blue">Experience</span> cyber warfare like never before.
          </p>
          <div className="flex flex-wrap justify-center gap-4 mt-8 animate-fade-in">
            <button 
              className="btn-primary flex items-center gap-2 px-12 py-5 text-2xl font-black shadow-xl transition-transform hover:scale-105 focus:scale-105"
              onClick={() => window.location.href = '/operations/arena'}
            >
        ...
        <div className="bg-background-light/80 rounded-2xl p-10 border border-primary/30 shadow-2xl mb-20 flex flex-col md:flex-row items-center gap-10 animate-fade-in backdrop-blur-md" style={{boxShadow:'0 0 32px #7f5af055, 0 2px 32px #0A030Fcc'}}>
          <div className="flex-1 flex flex-col gap-6 items-center justify-center">
            <h2 className="text-3xl font-extrabold text-primary flex items-center gap-3 mb-4 border-b-2 border-accent-blue/40 pb-2">
              <Shield className="h-8 w-8 text-primary" /> What Makes It Different?
            </h2>
            <p className="text-xl whitespace-pre-line text-center font-semibold mb-6 text-white" style={{textShadow:'0 2px 12px #7f5af0cc'}}>
            ...
            <ul className="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-8 w-full">
              {showcasePoints.map((point, idx) => (
                <li key={idx} className="flex flex-col items-center gap-3">
                  {point.icon}
                  <span className="font-bold text-lg text-accent-blue text-center drop-shadow-[0_2px_8px_#7f5af0cc]">{point.title}</span>
                  <span className="text-white text-center text-base font-normal opacity-90" style={{textShadow:'0 2px 8px #7f5af055'}}>{point.desc}</span>
                </li>
              ))}
            </ul>
          </div>
  ...
  {/* No timeline/leaderboard/irrelevant elements for pure marketing impact */}
        ...
        <div className="text-center mt-20 animate-fade-in">
          <h2 className="text-4xl font-extrabold text-primary mb-8 border-b-2 border-accent-blue/40 inline-block pb-2">Ready to Experience Cyber Combat?</h2>
          <p className="text-2xl text-white mb-10 max-w-2xl mx-auto font-semibold" style={{textShadow:'0 2px 12px #7f5af0cc'}}>
            This is your chance to <span className="text-primary">practice</span>, <span className="text-accent-blue">play</span>, and <span className="text-red-400">experience</span> cyber security like never before. Enter the arena. Make history.
          </p>
          <button 
            className="btn-primary px-16 py-6 text-2xl font-black flex items-center gap-4 shadow-xl animate-pulse-slow transition-transform hover:scale-105 border-2 border-accent-blue/40"
            onClick={() => window.location.href = '/operations/arena'}
          >
