...
const RedVsBluePage = () => {
  ...
  const [openRequests, setOpenRequests] = useState(dummyRequests);
  ...
  const handleSelectMode = (mode) => {
    ...
    if (mode === 'AI') {
      ...
      setStep('waiting');
      // Simulate partner joining after 3s
      setTimeout(() => {
  ...
  const handleJoinRequest = (req) => {
    setSelectedOp(dummyOperations.find(o => o.name === req.operation));
    setSelectedRole(req.team === 'Red' ? 'Blue' : 'Red');
    setSelectedMode('Player');
    setPartnerJoined(true);
    setLabQuestions(req.team === 'Red' ? blueQuestions : redQuestions);
    setStep('lab');
  };
