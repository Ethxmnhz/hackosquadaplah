...
// Professional Sidebar: All key sections, icons, badges, color theme
const Sidebar = ({ mobile = false, onClose }: SidebarProps) => {
  ...
  return (
    <aside
      className={`fixed z-40 top-0 left-0 h-full w-72 bg-gradient-to-br from-[#0A030F] via-background-dark to-[#181024] shadow-2xl border-r border-primary/20 transition-transform duration-300 ${mobile ? 'block' : isMinimized ? '-translate-x-64' : 'translate-x-0'}`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="flex items-center justify-between px-6 py-5 border-b border-primary/10">
        <Logo className="h-10 w-10" />
        <button onClick={toggleSidebar} className="text-primary hover:text-accent-blue transition-colors">
          <ChevronDown className="h-6 w-6" />
        </button>
      </div>
      <nav className="flex-1 px-4 space-y-2">
        {/* Main Navigation */}
        <NavSection title="Main" icon={<Home className="h-5 w-5 text-primary" />}>
          <NavItem to="/dashboard" icon={<Home className="h-5 w-5 text-primary" />} label="Dashboard" />
          <NavItem to="/red-vs-blue/operations" icon={<Flame className="h-5 w-5 text-red-400" />} label="Labs" />
          <NavItem to="/red-vs-blue/matchmaking" icon={<Sword className="h-5 w-5 text-accent-blue" />} label="Matchmaking" />
          <NavItem to="/leaderboard" icon={<Trophy className="h-5 w-5 text-yellow-400" />} label="Leaderboard" />
        </NavSection>
        {/* User Section */}
        <NavSection title="User" icon={<User className="h-5 w-5 text-accent-blue" />}>
          <NavItem to="/profile" icon={<User className="h-5 w-5 text-accent-blue" />} label="Profile" />
        </NavSection>
        {/* Admin Section */}
        {user && user.role === 'admin' && (
          <NavSection title="Admin" icon={<Shield className="h-5 w-5 text-primary" />}>
            <NavItem to="/admin/operations" icon={<Settings className="h-5 w-5 text-primary" />} label="Admin Panel" />
          </NavSection>
        )}
      </nav>
      {/* Sidebar footer: user info, sign out, etc. */}
      <div className="px-6 py-4 border-t border-primary/10 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-primary/70">{user?.role || 'Member'}</div>
        </div>
        <button onClick={signOut} className="text-red-400 hover:text-red-600 transition-colors" title="Sign out">
          <LogOut className="h-6 w-6" />
        </button>
      </div>
      {/* TODO: Integrate Topbar/Header for user info, notifications, quick actions */}
    </aside>
  );
